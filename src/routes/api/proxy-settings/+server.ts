...
function getProxySettings(db: ReturnType<typeof getDb>, userId: string): ProxySettings {
	...
	return {
		...
		headroom: getUserSetting<boolean>(db, userId, 'proxyHeadroom') === true
	};
}
...
const postSchema = z
	...
	.refine((data) => data.caveman !== undefined || data.headroom !== undefined, {
...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	const db = getDb();
	if (parsed.data.caveman !== undefined) {
		setUserSetting(db, user.id, 'proxyCaveman', parsed.data.caveman);
	}
	if (parsed.data.headroom !== undefined) {
		setUserSetting(db, user.id, 'proxyHeadroom', parsed.data.headroom);
	}
