...
import { upsertFetchedModels } from '$lib/server/db/repo/models.js';
...
export const POST: RequestHandler = async ({ locals, params }) => {
	...
	let modelIds: string[];
	try {
		modelIds = await fetchProviderModels(params.id);
	} catch (e) {
		error(502, { message: e instanceof Error ? e.message : 'Failed to fetch models' });
	}
	const result = upsertFetchedModels(db, params.id, modelIds);
	return json(result);
};
